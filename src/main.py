...
async def on_message(message):
	...
	elif command in ["balance", "bal", all_reg_commands_aliases["balance"]]:
	...
	elif command in ["deposit", "einzahlen", all_reg_commands_aliases["deposit"]]:
	...
	elif command in ["withdraw", "auszahlen", all_reg_commands_aliases["withdraw"]]:
		if "none" in param[1] or param[2] != "none": # we need 1 and only 1 parameter
			color = discord_error_rgb_code
	...
	elif command in ["give-item", "give"]:
		"""
		if not staff_request:
			color = discord_error_rgb_code
			embed = discord.Embed(description=f"🔒 Requires botmaster role", color=color)
	...
	elif command in ["inventory", "inv", "Rucksack"]:
		# handler
		try:
