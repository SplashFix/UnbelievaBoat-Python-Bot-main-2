...
class pythonboat_database_handler:
	...
	async def add_money(self, user, channel, username, user_pfp, reception_user, amount, recept_uname):
		...
		embed = discord.Embed(
			...
			color=color)
		embed.set_author(name=f"{username}´s Bank Account", icon_url=user_pfp)
		await channel.send(embed=embed)
	...
	async def check_inventory(self, user, channel, username, user_pfp):
		...
		if items == "none":
			...
			for i in range(len(items)):
				...
				if items[i][1] > 0:
					inventory_checkup += f"**{items[i][1]} - {item_display_name}**\n\n\n"

		color = self.discord_blue_rgb_code
		embed = discord.Embed(title="", description="**Tipp:** Mit `!use <item>` kannst du ein Item aus deinem Inv verbrauchen", color=color)
		embed.add_field(name="", value=f"{inventory_checkup}")
		embed.set_author(name=f"{username} Inventory", icon_url=user_pfp)
